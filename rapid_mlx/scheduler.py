...
class Scheduler:
    ...
    def _xml_tool_close_guard(self) -> XmlToolCloseGuard | None:
        """A fresh close guard for a tool request, when this model's wire needs one.
        ...
        if not hasattr(self, "_xml_close_guard_spec_cache"):
            ...
            if opt_out in ("0", "off", "false", "no"):
                logger.warning(
                    "xml tool-call close guard disabled by %s=%s",
                    XML_CLOSE_GUARD_OPT_OUT_ENV,
                    opt_out,
                )
            else:
                spec = xml_close_guard_spec(self._actual_tokenizer)
                if spec is not None:
                    logger.info(
                        "xml tool-call close guard armed: after %s only %s may follow",
                        spec.trigger,
                        [sorted(allowed) for _prefix, allowed in spec.states],
                    )
