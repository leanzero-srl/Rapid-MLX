...
class SchedulerConfig:
    ...
    # multiple of the engine's measured one-token forward -- the fixed cost
    # every forward pays -- instead of a fixed token count per row. A lone
    # prefill keeps ``prefill_step_size``. ``0`` disables the bound. Appended
    ...
    def __post_init__(self) -> None:
        if isinstance(self.fair_prefill_step_ratio, bool) or not (
            ...
            and self.fair_prefill_step_ratio >= 0
        ):
            raise ValueError("fair_prefill_step_ratio must be a finite number >= 0")
...
def _materialize_response_logprobs(responses: list[Any]) -> None:
    ...
    ``logprobs: true`` request (LeanZero lz.4). Scheduling every row on the
    owning thread is enough: a scheduled array is WAITED on from another
    thread, never re-scheduled onto its stream. ``async_eval`` keeps the step
    non-blocking — a blocking ``mx.eval`` here cost 0.5 ms/token (measured on
    a tiny MTP model, 900 -> 620 tok/s) on requests that never read the rows.
    """
    rows: list[mx.array] = []
...
def _install_mtp_vendored(
    ...
    yield_to_contention: bool = False,
) -> bool:
    ...
    * On the very first ``_step`` call we short-circuit and return the
    ...
    # Q-103: with ``yield_to_contention`` the singleton lock is taken only by
    # a request that reaches its first decode step ALONE. The scheduler reads
    # this attribute to keep admission open until then; without a capacity
    # attribute there is no lock to take, so the old batch-1 admission stays.
    _yields = bool(yield_to_contention and _nominal_completion_batch_size is not None)
    batch_gen._mtp_vendored_yields_to_contention = _yields

    def _other_work_in_flight() -> bool:
        prompt_batch = getattr(batch_gen, "_prompt_batch", None)
        try:
            prompting = prompt_batch is not None and len(prompt_batch) > 0
        except TypeError:
            prompting = False
        return prompting or bool(getattr(batch_gen, "_unprocessed_sequences", None))
    ...
    def _mtp_step():
        ...
        if not gb.uids or len(gb.uids) != 1:
            ...
            if _yields:
                # A row that has decoded beside another must never prime MTP
                # later from a mid-stream placeholder: plain for its life.
                for batch_uid in gb.uids:
                    if batch_uid not in _disabled_uids:
                        _log_mtp_initial_skip_once(
                            batch_uid, "decoding beside other requests"
                        )
                        _mark_disabled(batch_uid)
            return _orig_step()
        ...
        if _yields and uid not in _state and _other_work_in_flight():
            # Q-103: priming here would lock admission and park every other
            # request's prefill behind this whole decode.
            _stats["fallthrough_steps"] += 1
            _stats["ft_contended"] += 1
            _log_mtp_initial_skip_once(
                uid, "other requests are prefilling or queued at its first decode step"
            )
            _mark_disabled(uid)
            return _orig_step()
...
class Scheduler:
        ...
        if getattr(self.config, "spec_decode", "none") == "mtp":
            ...
            if (
                ...
                mtp_installed = _install_mtp_vendored(
                    ...
                    yield_to_contention=(
                        getattr(self.config, "mtp_yield_to_contention", False)
                        and not getattr(self.config, "mtp_continuous_batching", False)
                    ),
                )
                if mtp_installed:
    ...
    def _measure_forward_seconds(self) -> None:
        """Measure one warm one-token forward: the fixed cost of any step.

        Taken once, on the step thread, the first time a step is shared. A
        failure is kept and reported (``fair_prefill.forward_error``); the
        bound then stays off rather than guessing a cost.
        """
        ...
        try:
            ...
            self._fair_forward_s = sorted(samples)[len(samples) // 2]
            logger.info(
                "[fair_prefill] one-token forward %.4fs; a shared step targets "
                "%.3fs (x%s)",
                self._fair_forward_s,
                self._fair_forward_s * self.config.fair_prefill_step_ratio,
                self.config.fair_prefill_step_ratio,
            )
    ...
    def _apply_fair_prefill_size(self) -> int | None:
        """Bound a SHARED step's prompt chunk by measured time (Q-103).
        ...
        whole step. The per-row chunk is chosen so the step's wall time is
        ``fair_prefill_step_ratio`` one-token forwards, from the measured
        seconds per prompted token of recent full-chunk steps. Only ever
        lowers the size ``_apply_adaptive_prefill_size`` just set.
        """
        self._fair_chunk_size = None
        ratio = float(getattr(self.config, "fair_prefill_step_ratio", 0) or 0)
        bg = getattr(self, "batch_generator", None)
        if ratio <= 0 or bg is None:
            return None
        ...
        if self._fair_forward_s is None or self._fair_seconds_per_token is None:
            return None
        current = max(1, int(getattr(bg, "prefill_step_size", 1)))
        chunk = int(
            ratio * self._fair_forward_s / (self._fair_seconds_per_token * rows)
        )
    ...
    def _record_fair_prefill_step(
        ...
        """Keep seconds per prompted token from steps whose rows were all full.

        A full step (every prompted row consumed ``chunk`` tokens) carries no
        padding, so its wall time over its tokens is the cost the next shared
        step is sized from; decode rows riding in the same step are included,
        which is the cost those rows actually pay.
        """
        bg = getattr(self, "batch_generator", None)
        ...
        try:
            ...
            return
        if tokens > 0 and rows and chunk and tokens == rows * chunk:
    ...
    def _mtp_admission_open(self) -> bool:
        """Q-103: whether the ordinary vendored verifier leaves admission open.

        The verifier is batch-1 only once it OWNS a lone decode
        (``_mtp_vendored_admission_owner``). Until then a request's prefill
        and its first decode step are ordinary batched work, and the verifier
        installed with ``yield_to_contention`` sends any request that meets
        other work at its first decode step to plain decode for life. The
        continuous coordinator keeps its own attested capacity above.
        """
        if getattr(self.config, "mtp_continuous_batching", False):
            return False
        if not getattr(self.config, "mtp_yield_to_contention", False):
            return False
        batch_generator = getattr(self, "batch_generator", None)
        if not getattr(batch_generator, "_mtp_vendored_yields_to_contention", False):
            return False
        return getattr(batch_generator, "_mtp_vendored_admission_owner", None) is None
