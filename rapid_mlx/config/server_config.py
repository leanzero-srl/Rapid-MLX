...
# — ``rapid_mlx.engine`` eagerly imported ``engine_core``, which does
# ``import mlx.core`` at module scope — so naming ``BaseEngine`` here made
# ``rapid_mlx.config`` unimportable anywhere MLX is absent, and a wire
# adapter that consults ``get_config()`` died on Linux CI.
#
# The fix is in ``rapid_mlx/engine/__init__.py``, which now imports only the
...
class ServerConfig:
    ...
    # path (:func:`rapid_mlx.cli._serve_audio_mode`) sets this to True
