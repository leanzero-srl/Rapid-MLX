...
def _validate_logit_bias_finite(
    ...
    for _k, vv in v.items():
        ...
        # :mod:`rapid_mlx.middleware.exception_handlers` only sanitizes
...
# (``rapid_mlx/api/responses_adapter.py``) so the SDK-shorthand
...
# ``rapid_mlx/service/helpers.py``. Defined at module scope so both
...
class ChatCompletionRequest(BaseModel):
    ...
    # validator (``rapid_mlx/routes/chat.py`` ~L748) already 400s on that
...
# format added to :data:`rapid_mlx.routes.audio._TTS_CONTENT_TYPES`
...
#: ``rapid_mlx.audio.music._MAX_SECONDS`` — the engine enforces the same
...
class AudioMusicRequest(BaseModel):
    ...
    :class:`rapid_mlx.audio.music.MusicEngine` (MLX-native Stable Audio 3).
