...
Usage:
    # Start the server
    python -m rapid_mlx.server --model mlx-community/Llama-3.2-3B-Instruct-4bit

    # With MCP tools
    python -m rapid_mlx.server --model mlx-community/Qwen3-4B-4bit --mcp-config mcp.json
...
# ``rapid_mlx.server`` and register it manually on a stub FastAPI.
...
# stream — see ``rapid_mlx/service/helpers.py``.
...
# the body to disk. See rapid_mlx/routes/audio.py for the rationale.
...
def configure_cors_from_env(
    ...
    of the ``RAPID_MLX_CORS_*`` family; see ``rapid_mlx/cli.py``.
...
#
# All handler bodies live in ``rapid_mlx.middleware.exception_handlers``
...
def load_model(
    ...
    *,
    ...
    if _profile_modality == "video-gen":
        ...
        # ``python -m rapid_mlx.server --model <alias>`` bypasses
...
# and :func:`rapid_mlx.cli._serve_audio_mode`) imports the router lazily
# through ``rapid_mlx.routes.audio.register_audio_routes``. Removing the
...
def register_audio_routes_if_enabled() -> bool:
    ...
    * The loaded model alias / HF id resolves through the audio
      ...
      :func:`rapid_mlx.cli._serve_audio_mode` always populates
    ...
    * The operator passed ``--enable-audio`` on a text-mode boot
    ...
    ``python -m rapid_mlx.server`` entrypoint. Doing it here keeps the
...
Examples:
    ...
    # (rapid_mlx/cli.py) so this standalone entrypoint never becomes a
    ...
    # on the legacy ``python -m rapid_mlx.server`` entrypoint so the same
    ...
    # legacy ``python -m rapid_mlx.server`` entrypoint doesn't silently
    ...
    # (`python -m rapid_mlx.server` / `mise run`) reach the engine. Pre-0.6.52
