...
"""
CLI for rapid-mlx (package name: ``rapid_mlx``).
...
def _auth_feature_str(argv_api_key: str | None) -> str | None:
    ...
    """
    from rapid_mlx import server as _server
...
def _apply_body_receive_timeout_env(server_mod, *, logger=None) -> None:
    ...
    ``rapid_mlx.server`` reference each call without dragging the
...
def _port_preflight_or_die(host: str, port: int, *, model: str) -> None:
    ...
    ``python -m rapid_mlx.server`` entrypoint can call it too without
...
def _print_unknown_model_help(name: str, *, full_path_example: str) -> None:
    ...
    """
    from rapid_mlx.model_aliases import POPULAR_ALIASES, suggest_similar
...
def _load_embedding_model_or_exit(args, load_fn) -> None:
    ...
    (``rapid_mlx.server.load_embedding_model``) — passed in so tests
...
def _check_disk_space(
    ...
    *,
    ...
    from rapid_mlx._download_gate import mflux_missing_weights
...
def _gather_kv_cache_dtype_inputs(model_name: str) -> tuple[dict | None, dict | None]:
    ...
    :func:`rapid_mlx.kv_cache_dtype.resolve_kv_cache_dtype` still catches
...
def _try_mirror_prefetch(
    ...
    *,
    ...
    Delegates to :func:`rapid_mlx._mirror.download_with_mirror_fallback`.
    ...
    """
    from rapid_mlx.model_aliases import subfolder_allow_patterns
    ...
    try:
        from rapid_mlx._mirror import download_with_mirror_fallback
    except ImportError:
...
def _env_flag_active(raw: str | None) -> bool:
    ...
    """
    from rapid_mlx.model_metadata import env_flag_active
...
def _offline_complete_cached_snapshot(model_name: str):
    ...
    from rapid_mlx.model_metadata import resolve_offline_cached_snapshot
...
def serve_command(args):
    ...
    try:
        from rapid_mlx._download_gate import weightless_stub_notice
        from rapid_mlx.model_aliases import resolve_model as _resolve_model
    ...
    # the single SSOT — both this entrypoint and the ``rapid_mlx.server``
    ...
    # See rapid_mlx/middleware/body_size.py for the DoS rationale.
    _max_body_arg = getattr(args, "max_request_bytes", None)
    if _max_body_arg is not None:
        server._max_request_bytes = max(0, int(_max_body_arg))
    else:
    ...
    # ``rapid_mlx/server.py::configure_cors_from_env``.
    #
    ...
    from rapid_mlx.middleware.request_logging import install_request_logging_middleware
    ...
    # standalone ``python -m rapid_mlx.server`` entry behaves identically.
    ...
    # (#969) — ``python -m rapid_mlx.server`` calls the same helper so
    ...
    scheduler_config = SchedulerConfig(
        ...
        # helper (#969) — ``python -m rapid_mlx.server`` calls the same
    ...
    *,
    ...
    if profile is None:
        print(
            ...
            f"Only models listed in rapid_mlx/aliases.json can be submitted "
...
def bench_command(args):
    ...
    async def run_benchmark():
        ...
        scheduler_config = SchedulerConfig(
            ...
            # (rapid_mlx/routes/chat.py), and `rapid-mlx bench` never consumes
...
def _format_bytes(n: int) -> str:
    ...
    Aligned with ``rapid_mlx._download_gate._format_size`` so the same
...
def _external_model_roots() -> list[str]:
    ...
    if not raw:
        return []
    from rapid_mlx.model_aliases import _external_model_root_values
...
def _scan_external_model_dirs(
    ...
    :func:`rapid_mlx._download_gate._snapshot_is_complete`, the same check
    ...
    folder.
    """
    from rapid_mlx._download_gate import _snapshot_is_complete
...
def _cache_runnability(repo: str) -> bool | None:
    ...
    try:
        ...
        from rapid_mlx.model_metadata import (
...
def _cached_subfolder_size(repo_id: str, subfolder: str) -> int | None:
    ...
    from rapid_mlx.model_metadata import resolve_unreferenced_cached_subfolder
...
def _cached_models_json_payload() -> dict:
    ...
    from rapid_mlx.model_aliases import list_profiles, resolve_subfolder
...
def _available_models_json_payload() -> dict:
    ...
    """
    from rapid_mlx.model_aliases import list_builtin_aliases, list_profiles
    from rapid_mlx.model_sizes import size_bytes
    ...
    def _profile_dict(alias, p) -> dict:
        ...
        if modality == "image-gen":
            from rapid_mlx.image.engine import default_steps_for_model
...
def models_command(args):
    ...
    from rapid_mlx._version_check import print_staleness_warning_if_any
    from rapid_mlx.model_aliases import list_profiles
    from rapid_mlx.model_sizes import format_size
    ...
    # (``rapid_mlx/audio/aliases.json``), separate from the text profiles.
    #
    ...
    # (rapid_mlx/audio/aliases.json) feeds the same table so
    ...
    from rapid_mlx.model_aliases import list_builtin_aliases, user_alias_reserved_names
    from rapid_mlx.user_aliases import validated_user_aliases
...
def _sync_pulled_variant_marker(repo_id: str, variant: str | None) -> None:
    ...
    if variant is not None:
        ...
        try:
            from rapid_mlx._download_gate import persist_pulled_variant
    ...
    try:
        from rapid_mlx._download_gate import clear_pulled_variant
...
def _pull_repository(
    ...
    *,
    ...
    try:
        from rapid_mlx._download_gate import _format_size, reap_orphan_incomplete_blobs
...
def pull_command(args):
    ...
    from rapid_mlx.audio.registry import runtime_assets_for, runtime_requirements_for
...
def ps_command(_args):
    ...
    for proc in psutil.process_iter(["pid", "cmdline", "create_time"]):
        try:
            ...
            continue
        if not any(
            ("rapid-mlx" in c or "rapid_mlx" in c) and "serve" in cmd for c in cmd
        ):
            continue
...
def chat_command(args):
    ...
    # function and ``rapid_mlx/first_run.py``. Nudging once, after value has
    ...
    try:
        if getattr(args, "mcp_config", None):
            from rapid_mlx.chat_mcp import ChatMCPRuntime
    ...
    if _generated_any and _is_tty:
        try:
            from rapid_mlx.first_run import chat_agent_tip_text, claim_chat_agent_tip
...
def info_command(args):
    ...
    from rapid_mlx._download_gate import _format_size, estimate_repo_size_bytes
    from rapid_mlx.model_sizes import is_listed, size_bytes
...
def _print_dflash_status(alias: str, profile) -> None:
    ...
    """
    from rapid_mlx.spec_decode.capability import assess_method, profile_looks_like_4bit
...
def _print_ddtree_status(alias: str, profile) -> None:
    ...
    from rapid_mlx.spec_decode.capability import assess_method, profile_looks_like_4bit
    from rapid_mlx.speculative.ddtree.eligibility import have_runtime
...
def main():
    ...
    if (
        ...
        from rapid_mlx.first_run import FIRST_RUN_MODEL_SIZE, select_chat_default
    ...
    if _image_weight_precision is not None:
        from rapid_mlx.image.precision import resolve_image_weight_precision
    ...
    if (
        ...
        if resolved != args.model:
            ...
            if args.command == "share" and getattr(args, "quicksilver", False):
                from rapid_mlx.share.quicksilver import CATALOG_DEFAULT_ALIAS
    ...
    # ``rapid_mlx/_download_gate.py`` for the policy.
    #
        ...
        if not _auto_yes and _interactive:
            ...
            if (
                ...
                if os.environ.get("RAPID_MLX_WAN_MODEL_DIR"):
                    from rapid_mlx.model_aliases import resolve_profile as _rp
    ...
    if args.command == "serve":
        ...
        from rapid_mlx.community_bench.cli import (
        ...
        from rapid_mlx.run.cli import start_command
