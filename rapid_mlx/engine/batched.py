...
def _load_lazy_and_install_disk_stream(
    ...
    Raises ``rapid_mlx.disk_stream_patch.UnsupportedModelTypeError``
    immediately (before serving ever starts) if ``model_name``'s
    ``model_type`` has no registered
    ``rapid_mlx.registry.StreamingAdapter`` — no silent fallback to
...
class BatchedEngine(BaseEngine):
        ...
        *,
        ...
        Args:
            ...
            enable_disk_stream: Keyword-only. ``--disk-stream`` (PRD-
                ...
                installs ``rapid_mlx.disk_stream_patch`` on its MoE blocks
    ...
    async def _start_llm(self) -> None:
        ...
        if getattr(self, "_enable_disk_stream", False):
            ...
            # rapid_mlx/moe_fusion.py; measured +7% decode on
            ...
            # rapid_mlx/gdn_in_proj_fusion.py; measured +2.0%/+0.9% decode
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        if self._mllm_scheduler:
            ...
            for key in (
                ...
                "max_num_waiting_observed",
                "max_num_running_observed",
                "observed_running_with_waiter",
                "total_prompt_tokens",
