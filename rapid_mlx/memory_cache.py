...
def _vendored_cache_registry() -> dict[str, type]:
    ...
    from rapid_mlx.models.qwen4_exp_cache import QSAIndexCache, Qwen4ExpStateCache
...
def _vendored_cache_class_names(cache: list[Any]) -> set[str]:
    ...
    while pending:
        ...
        if type(item).__module__ in {
            "rapid_mlx.models.deepseek_v4_cache",
            "rapid_mlx.models.qwen4_exp_cache",
        }:
...
def _trim_cache_offset(cache: list[Any], trim_by: int) -> list[Any] | None:
    ...
    def has_deepseek_pooling(layer: Any) -> bool:
        if type(layer).__module__ == "rapid_mlx.models.qwen4_exp_cache":
            return type(layer).__name__ == "QSAIndexCache"
        if type(layer).__module__ == "rapid_mlx.models.deepseek_v4_cache":
