    ...
    *,
    ...
    if lazy:
        ...
        if adapter_path is not None:
            lazy_kwargs["adapter_path"] = str(adapter_path)
        result = _mlx_lm_load(
            model_name,
    ...
    # Preserve the historical two-argument call shape (no kwargs at all when
    # neither option is set) for downstream wrappers and tests that
    # instrument this internal dispatch boundary.
...
def _load_with_tokenizer_fallback(
    model_name: str, *, enable_dspark: bool = False, adapter_path: str | None = None
):
